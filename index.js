...
app.use('/images', express.static(path.join(__dirname, 'images'))); // Папка для улучшенных изображений
...
async function enhanceImage(imageUrl) {
  ...
  try {
    ...
    await sharp(imageBuffer)
      .resize({ width: 800, height: 1200, fit: 'contain', kernel: 'lanczos3', background: { r: 0, g: 0, b: 0, alpha: 0 } })
...
async function getGameCover(gameName, platforms, igdbCover) {
  ...
  return await enhanceImage(igdbCover.replace('t_thumb', 't_cover_big_2x')); // Используем t_cover_big_2x
}
