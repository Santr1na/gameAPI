...
app.get('/search', async (req, res) => {
  ...
  try {
    const body = `fields id, name, cover.url, aggregated_rating, release_dates.date, genres.name, platforms.name; search "${query}"; sort aggregated_rating desc; limit ${limit}; where aggregated_rating > 0;`;
    const response = await axios.post(igdbUrl, body, { headers: igdbHeaders, timeout: 5000 });
    const games = await Promise.all(response.data.map((game) => processShortGame(game)));
    res.json(games);
  } catch (error) {
